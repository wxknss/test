...
public class ElytraPlus extends Module {
    ...
    public void onEnable() {
        if (mc.player.getY() < infiniteMaxHeight.getValue() && mode.getValue() == Mode.Pitch40Infinite) {
            disable(isRu() ? "Поднимись выше " + Formatting.AQUA + infiniteMaxHeight.getValue() + Formatting.GRAY + " высоты!" : "Go above " + Formatting.AQUA + infiniteMaxHeight.getValue() + Formatting.GRAY + " height!");
        }
        flying = false;
        reset();
        infiniteFlag = false;
        acceleration = 0;
        accelerationY = 0;
        if (mc.player != null) height = (float) mc.player.getY();
        pingTimer.reset();
    ...
    private void doPreLegacy() {
        ...
        if (!mc.player.isFallFlying()) {
            if (hasTouchedGround && !mc.player.isOnGround() && mc.player.fallDistance > 0 && instantFly.getValue()) ThunderHack.TICK_TIMER = 0.3f;
    ...
    private void doSunriseNew() {
        ...
        int elytra = InventoryUtility.getElytra();
        if (elytra == -1) return;
        if (mc.player.isOnGround()) { mc.player.jump(); acceleration = 0; return; }
        if (mc.player.fallDistance <= 0) return;
        if (mc.options.jumpKey.isPressed() || mc.options.sneakKey.isPressed()) { acceleration = 0; takeOnElytra(); }
        else {
            takeOnChestPlate();
            if (mc.player.age % 8 == 0) matrixDisabler(elytra);
    ...
    private void doMotionPacket(EventMove e) {
        ...
        if (mc.player.horizontalCollision && (ncpStrict.is(NCPStrict.New) || ncpStrict.is(NCPStrict.Motion)) && mc.player.age % 2 == 0) e.setY(-0.07840000152587923);
    ...
    private void takeOnElytra() {
        ...
        if (elytra == -1) return;
        elytra = elytra >= 0 && elytra < 9 ? elytra + 36 : elytra;
        if (elytra != -2) { clickSlot(elytra); clickSlot(6); clickSlot(elytra); sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING)); }
    }

    private void takeOnChestPlate() {
        int slot = getChestPlateSlot();
        if (slot == -1) return;
        if (slot != -2) { clickSlot(slot); clickSlot(6); clickSlot(slot); }
    }
