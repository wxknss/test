...
public class OCScaffold extends Module {
    ...
    private final Setting<Integer> expand = new Setting<>("Expand", 3, 0, 5);
    ...
    public void onTick(EventTick e) {
        ...
        if (targetPos != null && timer.passedMs(delay.getValue())) {
    ...
    public void onMove(EventMove e) {
        if (fullNullCheck()) return;
        if (targetPos != null && rotate.getValue()) {
            e.cancel();
        }
    }
    ...
    private void findBlock() {
        ...
        if (targetPos == null && expand.getValue() > 0) {
            double yawRad = Math.toRadians(mc.player.getYaw());
            double dirX = -Math.sin(yawRad);
            double dirZ = Math.cos(yawRad);
            
            for (int i = 1; i <= expand.getValue(); i++) {
