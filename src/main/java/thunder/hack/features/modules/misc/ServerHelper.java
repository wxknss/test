...
public class ServerHelper extends Module {
    ...
    public boolean justSolvedMath = false;
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.getPacket() instanceof GameMessageS2CPacket pac) {
            ...
            if (photomath.getValue()
                ...
                try {
                    int result = Integer.parseInt(StringUtils.substringBetween(pac.content().getString(), "Решите: ", " + "))
                            + Integer.parseInt(StringUtils.substringBetween(pac.content().getString(), " + ", " кто первый"));
                    justSolvedMath = true;
                    mc.player.networkHandler.sendChatMessage(String.valueOf(result));
            ...
            if (antiTpHere.getValue()) {
                ...
                if ((msg.contains("Телепортация...") || msg.contains("Телепортирование...")) && check(msg)) {
                    flag = true;
                    atphtimer.reset();
                }
            }
